	...
	<table width="100%" border=0 cellpadding=0 cellspacing=0>
		<tr>
			<td colspan="10">
				...
				<li ><a class='current' href="<?= ADMIN?>productos/index.php?accion=colores" >Colores</a></li>
				<li ><a  href="<?= ADMIN?>productos/index.php?accion=talles" >Talles</a></li>
...
<table cellpadding=5 cellspacing=0 border=1 width="90%" align="center" >
	...
	<tr class="<?=($contador%2==0? "fila_par":"fila_impar");?>">
		...
		<td >
		<? if($color['activo'] == 1) echo"Activo";else echo "No Activo";?>
		</select>
		</td>

		<td><a href="index.php?accion=colores&id=<?= $color['id'] ?>">
	...
	<? endforeach ?>
	<tr>
		<td align="right" colspan="10"><a href="index.php?accion=colores&id=0">
...
<? if(!@$_GET["id"]){
	$id_color = 0;	
	$msj_1 = "INGRESE NUEVO COLOR";
	}else{
	$msj_1 = "MODIFICAR COLOR SELECCIONADO";
	$id_color = @$_GET["id"];		
	$color = @Producto::get_color_by_id($_GET["id"]);
	$nombre = $color["nombre"];
	$activo = $color["activo"];
	}	
...
<form name="disco" method="post" enctype="multipart/form-data" action="index.php?accion=colores_admin">
...
<tr><td><br></td></tr>
	...
	<tr>
		...
		<option value="1" <? if($color['activo'] == 1) echo"selected";?>>Activado</option>
		<option value="0"  <? if($color['activo'] == 0) echo"selected";?>>Desactivado</option>
		</select>
		</td>
	</tr>
