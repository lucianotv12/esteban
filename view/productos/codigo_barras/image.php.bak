...
/* Default value */
if (!isset($output))  $output   = $_GET["output"]; 
if (!isset($barcode)) $barcode  = $_GET["code"];
if (!isset($type))    $type     = $_GET["type"];
if (!isset($width))   $width    = $_GET["width"];
if (!isset($height))  $height   = $_GET["height"];
if (!isset($xres))    $xres     = $_GET["xres"];
if (!isset($font))    $font     = $_GET["font"];
/*********************************/ 
